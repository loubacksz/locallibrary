...
exports.book_update_post = [
    ...
    async (req, res, next) => {
        try {
            ...
            await Book.update(
                {
                    ...
                    summary: req.body.summary,
                    isbn: req.body.isbn,
                    authorId: authorRaw.dataValues.id
                },
                {
                    where: {
                        id: req.params.id,
            ...
            req.body.genre.forEach(async (formGenreId) => {
                if (!genresId.includes(formGenreId)) {
                    try {
                ...
                } else if (genresId.includes(formGenreId)) {
            ...
            // check errors - all this is just to render the form again with the same values passed for the first time | if true - send to book.url with errors message
            if (!errors.isEmpty()) {
                // Get all authors and genres for form
                const [allAuthorsRaw, allGenresRaw] = await Promise.all([
                    Author.findAll({ order: [['first_name', 'ASC']] }),
